var provinces = ['北京市','上海市','天津市','重庆市','河北省','山西省','辽宁省','吉林省','黑龙江省','江苏省','浙江省','安徽省','福建省','江西省','山东省','河南省','湖北省','湖南省','广东省','海南省','四川省','贵州省','云南省','陕西省','甘肃省','青海省','内蒙古自治区','广西壮族自治区','西藏自治区','宁夏回族自治区','新疆维吾尔自治区','台湾','香港','澳门'];
...
function set_province(province,val_p){
      province.options[0] = new Option();
      province.options[0].text = "--请选择省份--";
      for(i=0; i<provinces.length; i++){
         province.options[i+1] = new Option();
         province.options[i+1].text = provinces[i];
         province.options[i+1].value = provinces[i];
         if(val_p == provinces[i]){
            province.options[i+1].setAttribute("selected","selected");
         }
      }
}
function set_city(province, city,val_c)
{
   var pv, cv;
   var i;
   pv=province.value;
   ...
   if(typeof(cities[pv])=='undefined'){
      ...
      for(i=0; i<cities[pv].length; i++){
         ...
         if(val_c == cities[pv][i]){
            city.options[i].setAttribute("selected","selected");
         }
      }
   }

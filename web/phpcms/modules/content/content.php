...
class content extends admin {
	...
	public function add_info(){
		$model = pc_base::load_model('ttx_content_model');
		
		if(isset($_POST['dosubmit'])) {
			// var_dump($_POST);exit();
			$arr['c_id'] = $id = $_POST['id'] = intval($_POST['id']);
			$arr['cat_id'] = $catid = $_POST['catid'] = intval($_POST['catid']);
			$info = $_POST['info'];
			foreach ($info['name'] as $key => $value) {
				$arr['name'] = $value;
				$arr['content'] = $info['content'][$key];
				if(!isset($info['id'][$key]) OR $info['id'][$key]==''){
					$id = $model->insert($arr);
				}else{
					$id = $model->update($arr , array('id'=>$info['id'][$key]));
				}	
			}
			if(isset($_POST['dosubmit'])) {
			...
			$where = array('c_id'=>$id,'cat_id'=>$catid);
			$result = $model->select($where , '*','','id');
			// var_dump($result);exit;
			include $this->admin_tpl('add_info');
		}
	}

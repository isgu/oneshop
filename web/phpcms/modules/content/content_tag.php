...
class content_tag extends admin {
	private $db;
	public $siteid,$categorys;
	public function __construct() {
		parent::__construct();
		$this->db = pc_base::load_model('content_tag_model');
	...
	public function init() { 
			...
			$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=content&c=content_tag&a=add&type=' . $type . '\', title:\''.L('add').'\', width:\'340\', height:\'100\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('add'));

			$infos = $this->db->listinfo("type='$type'", 'id desc', ''); 
			include $this->admin_tpl('content_tag_list'); 
	}
	public function add() {
		...
		if(isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
			...
			$type = $_GET['type'];
			include $this->admin_tpl('content_tag_add'); 
		}
	}
	...
	public function edit() {
		...
		if(isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) { 
			$id = $this->db->update($_POST['info'] , array('id'=>$_POST['id']));
			showmessage(L('operation_success'),HTTP_REFERER,'', 'edit');
		} else {
			$id = $_GET['id'] = intval($_GET['id']); 
			$where = array('id'=>$id);
			$info = $this->db->get_one($where , '*','','id');
			include $this->admin_tpl('content_tag_edit'); 
		}
	}
	/**
	 * 删除
